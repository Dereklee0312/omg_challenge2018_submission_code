...
class conv_3d_id_model:
    ...
    def create(self):
        main_input = Input(
            shape=(self.seq_len, self.img_x, self.img_y, self.ch_n), name="main_input"
        )

        layer = Conv3D(32, kernel_size=(3, 3, 3), activation="relu", padding="same")(
            main_input
        )
        layer = Conv3D(32, kernel_size=(3, 3, 3), activation="relu", padding="same")(
            layer
        ...
        layer = Conv3D(64, kernel_size=(3, 3, 3), activation="relu", padding="same")(
            layer
        )
        layer = Conv3D(64, kernel_size=(3, 3, 3), activation="relu", padding="same")(
            layer
        ...
        reg_conv_3d_model_double_in = Model(
            inputs=[main_input, aux_input], outputs=[reg_out]
        )
