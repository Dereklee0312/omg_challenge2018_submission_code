...
def build_model_LSTM():
    ...
    if lstm_attention:
        ...
        lstm_output, _ = AttentionWeightedAverage(
            name="attlayer", attention_type=attention_type
        )(lstm_output)
    else:
        lstm_output = LSTM(lstm_output_dim, return_sequences=False)(seq_input_drop)

    second_last = Dropout(final_dropout)(lstm_output)
    second_last_drop = Dense(
        second_last_dim, name="second_last", activation=activation
    )(second_last)
...
def build_model(time_step_length, N_features):
    ...
    model.add(
        Conv1D(
            ...
            kernel_size,
            activation="relu",
            input_shape=(time_step_length, N_features),
        )
    )
